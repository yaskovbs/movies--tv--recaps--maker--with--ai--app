...
**פלטפורמת אינטרנט ליצירת סיכומי וידאו מקצועיים לסרטים וסדרות באמצעות בינה מלאכותית.** מעלים סרטון, מתארים אותו בטקסט, ו-Google Gemini AI צופה בסרטון ובוחר בעצמו את הרגעים המשמעותיים ביותר לחיתוך — הכול ישירות בדפדפן, בלי להעלות את קובץ הווידאו לשום שרת.

איך המערכת עובדת מאחורי הקלעים (הזרימה המלאה, Gemini, Supabase, עמידות ונפילות חזרה) מתועד ב-[ARCHITECTURE.md](ARCHITECTURE.md). תיעוד מפורט של כל השינויים וההחלטות הטכניות לאורך הפיתוח נמצא בקובץ [CHANGELOG.md](CHANGELOG.md).
...
- 🤖 **בחירת רגעים חכמה עם Gemini AI** — Gemini צופה בסרטון המקורי בעצמו ובוחר את הרגעים המשמעותיים ביותר לחיתוך (נקודות מפנה בעלילה, רגעים דרמטיים) במקום חיתוך אקראי במרווחים קבועים; אם זה נכשל, המערכת נופלת אוטומטית לחיתוך במרווח קבוע.
...
- 💾 **שמירה אמיתית של סיכומים** — ניתן לשמור סיכום (וידאו + אודיו) להיסטוריה שלכם ולחזור אליו מאוחר יותר, כולל נגן וידאו מובנה בעמוד ההיסטוריה.
...
4. מלאו כותרת ותיאור מפורט של הסרט/הסדרה — התיאור עוזר ל-Gemini לבחור את הרגעים הנכונים לחיתוך.
...
7. לאחר הסיום: צפו בסיכום, הורידו את הקובץ, או שמרו אותו להיסטוריה.
...
| בינה מלאכותית | Google Gemini (צפייה בווידאו ובחירת רגעים לחיתוך) |
...
- סיכומים שנשמרים (וידאו, אודיו) מאוחסנים דרך Supabase לפי בחירת המשתמש בלבד. בלי התחברות, כל מבקר מקבל זהות אנונימית פרטית (Supabase Anonymous Auth) - ההיסטוריה שלו לא נגישה למבקרים אחרים.
